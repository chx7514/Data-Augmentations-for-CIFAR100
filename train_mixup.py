...
if args.resume:
    ...
    assert os.path.isfile('./best_model_mixup_' + args.model + '.pth'), 'Error: no checkpoint directory found!'
    checkpoint = torch.load('./best_model_mixup_' + args.model + '.pth')
...
path = os.path.join('./path/to/log/mixup', args.model)
...
for epoch in range(start_epoch, num_epochs):
    _, train_log = mixup_train(train_loader, model, criterion, optimizer, epoch)
    ...
    if is_best:
        ...
        'acc': best_acc,
        }, False, 'best_model_mixup_' + args.model + '.pth')
